...
export default function Page() {
  ...
  // Lista de termos impróprios (expansível)
  ...
  // Função INTELIGENTE para gerar frases (Resolve o problema da Maiúscula)
  ...
  const renderDynamicHeadline = (name: string, sentence: string) => {
    ...
    if (!name || !name.trim() || containsProfanity(name)) {
      ...
      // Retorna a frase com a primeira letra forçada em MAIÚSCULA
    ...
    // Se tem nome válido, retorna "Nome, frase" (frase mantida em minúscula)
  ...
  // Lógica auxiliar para limpar campos dependentes no modo detalhado

  const checkDetailedLogic = () => {

      // (Lógica simplificada mantida do original para consistência)
  ...
  const calcular = () => {
    ...
    // 2. Validação de Profanidade (Estendida para todos os campos de texto)
